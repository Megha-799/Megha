...
class TestLoanAPI(unittest.TestCase):
	...
	def test_3_save_farmer_details(self):
		...
		response = farmer_details(
			action="save", application_id=app_id, full_name="John", last_name="Doe", mobile_phone="0911223344"
		)
		self.assertEqual(response["status"], "success")
	...
	def test_4_application_manager_review(self):
		...
		response = application_manager(action="review", application_id=app_id)
		self.assertEqual(response["status"], "success")
	...
	def test_7_get_consent_data_success(self):
		...
		sample_consent = {
			"consent": {"consent_creation_request_id": "REQ-123"},
			"farmer": {"id": "F-001", "name": "Jane Doe Basic"},
			"selected_data": {
				"farmer": {
					"given_name": "Jane",
					"family_name": "Doe",
	...
	def test_8_submit_missing_consent(self):
		...
		response = application_manager(action="submit", application_id=app_id)
	...
	def test_9_cancel_application(self):
		...
		response = application_manager(
			action="cancel", application_id=app_id, reason="User requested cancellation"
		)
		self.assertEqual(response["status"], "success")
	...
	def test_11_get_loan_applications(self):
		...
		from oan_a2c.openagrinet_access_to_credit.doctype.loan_application.loan_application import (
