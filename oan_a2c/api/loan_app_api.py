	...
	if not kwargs or list(kwargs.keys()) == ["cmd"]:
		...
		try:
			if (
				...
				and hasattr(frappe.local.request, "form")
				and frappe.local.request.form
			):
...
def create_loan_from_lead(**kwargs):
	...
	try:
		...
		return success(
			{
				"application_id": loan_app_doc.name,
				"message": "Loan Application created successfully from Lead.",
			}
		)
...
_LOAN_DETAIL_FIELDS = [
	...
	"woreda",
	"kebele",
	"harvest_aggregator_type",
	"name_of_cooperative",
	"dap_quantity_kg",
	"urea_quantity_kg",
	"unit_price_per_fertilizer_type",
	"total_fertilizer_cost",
	"type_of_agrochemical",
	"agrochemical_quantity_requested",
	"agrochemical_unit_price",
	"total_crop_protection_cost",
	"selected_input_supplier",
	"upfront_contribution_male_percent",
	"upfront_contribution_female_percent",
	"crop_insurance_premium_percent",
]
...
_BANK_FIELDS = [
	...
	"bank_account_no",
	"bank_name",
	"bank_swift_ifsc_code",
	"mobile_account_name",
	"mobile_payments_number",
	"total_amount_borrowing",
	"tax_id",
]
...
def supporting_documents(**kwargs):
	...
	if action == "delete":
		...
		if not application_id:
			return error("MISSING_APP_ID", "application_id is required.")
		file_owner = frappe.db.get_value(
			"File", document_id, ["attached_to_doctype", "attached_to_name"], as_dict=True
		)
		if (
			not file_owner
			or file_owner.attached_to_doctype != "Loan Application"
			or file_owner.attached_to_name != application_id
		):
...
_FARMER_FIELDS = [
	...
	"size_of_family",
	"number_of_children",
	"no_of_females_family",
	"no_of_males_family",
	"family_member_owns_land",
	"source_of_income",
	"education_level",
	"total_farmland_as_landowner",
	"total_farmland_as_crop_sharing",
	"total_farmland_as_rented",
	"certification_id",
	"farmland_size_hectares",
	"land_ownership_status",
	"soil_fertility_minerals",
	"moisture_levels",
]
...
def get_consent_data(**kwargs):
				...
				return success(
					{
						...
						"farmer_details": {
							"full_name": f"{getattr(doc, 'full_name', '')} {getattr(doc, 'last_name', '')}".strip()
							or None,
	...
	try:
		...
		return error("PARSE_ERROR", f"Failed to parse consent data: {e!s}")
			...
			return success(
				{
					...
					"farmer_info": {
						...
						"display": f"{farmer_name} — {loan_type} • {loan_months} Months"
						if loan_type
						else farmer_name,
					},
